...
Vector3 Transform(const Vector3& vector, const Matrix4x4& matrix) {
	...
	result.x = matrix.m[0][0] * vector.x / matrix.m[3][3];
	result.y = matrix.m[1][1] * vector.y / matrix.m[3][3];
	result.z = matrix.m[2][2] * vector.z / matrix.m[3][3];
...
void VectorScreenPrintf(int x,int y,const Vector3& vector, const char* name) {
	...
	Novice::ScreenPrintf(x + kRowHeight* 3, y, "%.02f", vector.y);
	Novice::ScreenPrintf(x + kRowHeight * 6, y, "%.02f", vector.z);
	Novice::ScreenPrintf(x + kRowHeight * 9, y, "%s",name);
}
